...
gp_co2 = MyGPR(kernel=kernel, alpha=1e-4, n_restarts_optimizer=1000,normalize_y=True)
gp_ch4 = MyGPR(kernel=kernel, alpha=1e-4, n_restarts_optimizer=1000, max_iter=30000,normalize_y=True)
...
r2_co2_gp = r2_score((y1_pred-y1_g),10**(sigma_co2))
r2_ch4_gp = r2_score((y2_pred-y2_g),10**(sigma_ch4))
print("Correlation coefficient",r2_co2_gp,r2_ch4_gp)
...
for i in range(X1_size):
    for j in range(T_size):
        ...
        os.system("echo 'Pressure,Temperature,X_co2,GP_co2,GP_ch4,Actual_co2,Actual_ch4,error_co2,error_ch4' >> adsorption_"+str(mol)+"_"+str(T)+".csv")
