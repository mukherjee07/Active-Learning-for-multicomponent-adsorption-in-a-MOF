...
for i in range(X1_size):
    ...
    mol = mol.replace("]","")
    mol = mol.replace("[","")
    os.system("rm error_"+str(mol)+".csv")
    os.system("touch error_"+str(mol)+".csv")
    os.system("echo 'Temp,Pressure,error_h2s,error_co2' >> error_"+str(mol)+".csv")
    for j in range(T_size):
        ...
        for k in range(P_size):
            ...
            e1 = ((GP_co2[k,j,i] - y1_g[Loc])/(y1_g[Loc] + 1E-3))*100
            e2 = ((GP_ch4[k,j,i] - y2_g[Loc])/(y2_g[Loc] + 1E-3))*100
            P = float(P_test[k])
            P = round(P,3)
            if (P == 0):
                P = 1e-6
            ...
            os.system("echo "+str(T_test[j])+","+str(P)+","+str(e1)+","+str(e2)+" >> error_"+str(mol)+".csv")
            os.system("sed -i 's/[][]//' error_"+str(mol)+".csv")
            os.system("sed -i 's/[][]//' error_"+str(mol)+".csv")
            os.system("sed -i 's/[][]//' error_"+str(mol)+".csv")
            os.system("sed -i 's/[][]//' error_"+str(mol)+".csv")
            os.system("sed -i 's/[][]//' error_"+str(mol)+".csv")
