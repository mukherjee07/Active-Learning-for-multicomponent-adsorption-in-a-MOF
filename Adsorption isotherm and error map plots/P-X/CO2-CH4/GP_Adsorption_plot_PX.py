...
gp_co2 = GaussianProcessRegressor(kernel=kernel_co2, alpha=1e-4,n_restarts_optimizer=1000, normalize_y=True)
gp_ch4 = GaussianProcessRegressor(kernel=kernel_ch4, alpha=1e-4,n_restarts_optimizer=1000, normalize_y=True)
...
P_test = 10**(P_test)
GP_co2 = np.ones((P_size,X1_size))
GP_ch4 = np.ones((P_size,X2_size))
...
for i in range(X1_size):
	mol = round(0.02+(i/50),3)
	os.system("rm adsorption_"+str(mol)+".csv")
	os.system("touch adsorption_"+str(mol)+".csv")
	os.system("echo 'Pressure,GP_co2,GP_ch4,Actual_co2,Actual_ch4,error_co2,error_ch4' >> adsorption_"+str(mol)+".csv")
	
	for j in range(P_size):
		Loc = (j*X1_size) + i
		GP_co2[j,i] = y1_pred[Loc]
		GP_ch4[j,i] = y2_pred[Loc]
		os.system("echo "+str(P_test[j])+","+str(GP_co2[j,i])+","+str(GP_ch4[j,i])+","+str(y1_g[Loc])+","+str(y2_g[Loc])+","+str(e1_g[Loc])+","+str(e2_g[Loc])+" >> adsorption_"+str(mol)+".csv")
		os.system("sed -i 's/[][]//' adsorption_"+str(mol)+".csv")
		os.system("sed -i 's/[][]//' adsorption_"+str(mol)+".csv")
		os.system("sed -i 's/[][]//' adsorption_"+str(mol)+".csv")
		os.system("sed -i 's/[][]//' adsorption_"+str(mol)+".csv")
		os.system("sed -i 's/[][]//' adsorption_"+str(mol)+".csv")
