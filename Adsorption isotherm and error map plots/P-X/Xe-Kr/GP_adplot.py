...
plt.plot(p,real_co2, marker = "o", markersize='10', markeredgecolor='k',linewidth='5',color = "cyan", linestyle = ":",label="$Xe$ (GCMC)")
plt.plot(p,gp_co2, marker = "o", markersize='10', markeredgecolor='k', linewidth='5',color = "grey", linestyle = ":",label="${Xe}$ (GP)")
plt.fill_between(p, real_co2 - (error_co2), real_co2 + error_co2,color='gray', alpha=0.2,label="95% Confidence")
#plt.errorbar(c, Xe,  yerr = Xe_Error, label="$\sigma_{GCMC}$",color = 'k', linestyle='')
#plt.xscale('log')
#plotting for 202 K
#plt.plot(Pressure202,GP_202, marker = "H",color="m",Linestyle="solid",label="GP (T = 202 K)")

plt.plot(p,real_ch4, marker = "s", markersize='10',markeredgecolor='k',linewidth='5', color = "red", linestyle = "-.",label="$Kr$ (GCMC)")
plt.plot(p,gp_ch4, marker = "s", markersize='10', markeredgecolor='k',linewidth='5',color = "orange", linestyle = ":",label="$Kr$ (GP)")
...
plt.title("Adsorption isotherm at $X_{Xe}$ = XXX",fontsize = "36")
plt.grid(visible=True,which='minor',color='#999999',linestyle='-',alpha=0.2)
#plt.ylabel("$CO_2$ adsorption in molecules/cylinder vol.($\AA^3$)", fontsize = "20")
plt.ylabel("$Xe$ and $Kr$ uptake in mg/g", fontsize = "36")
...
#plt.xscale('log')
plt.ylim(0,1500)
plt.savefig('AL_limit_Adsorption_Xe_XXX.png',dpi=300)
plt.show()
